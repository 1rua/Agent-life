...
def test_all_three_exposure_modes_share_routes_and_verified_core_result():
    ...
    for mode in EXPOSURE_MODES:
        core = FakeCore()
        exposure = create_gateway_exposure(mode, core=core, host_version="1.0.0")
...
class RawRequest:
    def __init__(self, body=b"{}"):
        self.method = "POST"
        self.url = "/agent-life/v2/negotiate"
...
def test_raw_host_boundary_requires_verifier_and_enforces_body_limit():
    core = FakeCore()
    route = next(item for item in create_gateway_exposure("host-route", core=core, host_version="1.0.0").routes if item.path == "/agent-life/v2/negotiate")
    ...
    exposure = create_gateway_exposure(
        "host-route", core=core, host_version="1.0.0", max_body_bytes=1,
...
def test_raw_host_boundary_passes_exact_bytes_to_verifier_before_core():
    ...
    exposure = create_gateway_exposure("direct-tls", core=core, host_version="1.0.0", verify_request=verifier)
    route = next(item for item in exposure.routes if item.path == "/agent-life/v2/negotiate")
    response = RawResponse()
    route.handler(RawRequest(b"{}"), response)
    assert response.status_code == 200
    assert seen[0]["body"] == b"{}"
    assert core.requests[0]["target"] == "/agent-life/v2/negotiate"
...
def test_verified_exposure_seam_reaches_the_independent_python_core(tmp_path):
    ...
    exposure = create_gateway_exposure("loopback-reverse-proxy", core=core, host_version="1.0.0")
