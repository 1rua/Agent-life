...
class ConversationClient(private val http: GatewayHttpClient) {
    ...
    fun rawEvents(): Flow<com.agentlife.gateway.events.GatewayEvent> = http.events()
    ...
    suspend fun readThreads(cursor: String? = null, limit: Int? = null): List<ConversationThread> {
        val response = execute(
            method = "GET",
            target = "/agent-life/v2/conversations" + query(
                "cursor" to cursor,
    ...
    suspend fun readConversation(conversationId: String): ConversationDetail {
        val response = execute(
            method = "GET",
            target = "/agent-life/v2/conversations/$conversationId",
        )
        if (response.status != 200) {
    ...
    suspend fun readTimeline(
        ...
        val response = execute(
            method = "GET",
            target = "/agent-life/v2/conversations/$conversationId/messages" + query(
                "cursor" to cursor,
                "limit" to limit?.toString(),
            ),
        )
    ...
    suspend fun submitBatch(
        ...
        val response = execute(
            method = "POST",
            target = "/agent-life/v2/conversations/$conversationId/message-batches",
            body = Json.canonical(Json.of(payload)).toByteArray(Charsets.UTF_8),
        )
        if (response.status !in 200..299) {
    ...
    suspend fun cancelGeneration(
        ...
        val response = execute(
            method = "POST",
            target = "/agent-life/v2/conversations/$conversationId/generations/$generationId/cancel",
