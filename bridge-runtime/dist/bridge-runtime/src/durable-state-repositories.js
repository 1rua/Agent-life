...
import { applyNotificationFilter, validateNotificationFilter, validateNotificationRecord, } from "../../bridge-contract/src/notification-store.js";
import { BridgeServiceError, equalIdentity, freezeRecord, identityKey, sessionKey, } from "../../bridge-contract/src/service-types.js";
const error = (code) => new BridgeServiceError(code);
...
const decimal = (value, code, allowNegative = false) => {
    ...
    try {
        ...
        if (caught instanceof BridgeServiceError)
            throw caught;
        throw error(code);
    }
};
const assertSession = (value) => {
    ...
    for (const field of ["tenantId", "humanPrincipalId", "deviceId", "agentInstanceId", "workspaceId", "sessionId"]) {
        nonEmpty(value[field], "DURABLE_SESSION_INVALID");
    }
    if (value.jobId !== undefined)
        nonEmpty(value.jobId, "DURABLE_SESSION_INVALID");
...
const decodeSession = (value, code = "DURABLE_SESSION_STATE_INVALID") => {
...
const validateTicket = (value) => {
    ...
    for (const field of ["ticketId", "tenantId", "humanPrincipalId", "deviceId", "bridgeFingerprint"]) {
...
const encodePairingInput = (value) => Object.freeze({
    tenantId: value.tenantId,
    humanPrincipalId: value.humanPrincipalId,
    deviceId: value.deviceId,
...
const decodePairingInput = (value) => {
...
const encodeNotification = (record) => {
...
const decodeNotification = (value, code = "DURABLE_NOTIFICATION_STATE_INVALID") => {
    if (!isRecord(value) || !exactKeys(value, [
...
const comparePosition = (left, right) => left.sourceEpoch === right.sourceEpoch
    ? (left.cursor < right.cursor ? -1 : left.cursor > right.cursor ? 1 : 0)
    : (left.sourceEpoch < right.sourceEpoch ? -1 : 1);
const sameJson = (left, right) => JSON.stringify(left) === JSON.stringify(right);
const notificationKey = (session, recordId) => `${sessionKey(session)}\u0000${recordId}`;
export class DurablePairingRepository {
    ...
    async acceptVerified(candidate) {
        validateTicket(candidate);
        const now = this.#clock();
        if (!Number.isFinite(now))
            throw error("PAIRING_CLOCK_INVALID");
        return runDurableBridgeTransaction(this.#store, "pairing.accept", async (transaction) => {
            ...
            if (rawTicket !== null) {
                ...
                if (!sameJson(encodeTicket(stored), encodeTicket(candidate)))
                    throw error("PAIRING_TICKET_TAMPERED");
                if (rawTicket.consumed)
                    throw error("PAIRING_TICKET_REPLAY");
            }
            ...
            if (previous && previous.bridgeFingerprint !== candidate.bridgeFingerprint)
                throw error("PAIRING_BINDING_MISMATCH");
            if (previous && candidate.pairingGeneration < previous.pairingGeneration)
                throw error("PAIRING_GENERATION_ROLLBACK");
            if (previous && candidate.pairingGeneration > previous.pairingGeneration + 1n)
                throw error("PAIRING_GENERATION_GAP");
...
export class DurableNotificationRepository {
    ...
    async append(session, record) {
        ...
        return runDurableBridgeTransaction(this.#store, "notification.append", async (transaction) => {
            ...
            if (rawExisting !== null) {
                ...
                if (!sameJson(rawExisting.record, encoded))
                    throw error("NOTIFICATION_RECORD_CONFLICT");
                return false;
            }
            ...
            if (rawPosition !== null) {
                if (!isRecord(rawPosition) || !exactKeys(rawPosition, ["cursor", "recordId", "sessionKey", "sourceEpoch"])
                    || rawPosition.sessionKey !== binding)
                    throw error("DURABLE_NOTIFICATION_STATE_INVALID");
                const previous = {
    ...
    async read(session, limit, filter) {
        assertSession(session);
        ...
        return runDurableBridgeTransaction(this.#store, "notification.read", async (transaction) => {
            ...
            for (const entry of await transaction.scan("notification.records")) {
                if (!isRecord(entry.value) || !exactKeys(entry.value, ["record", "sessionKey"]))
                    throw error("DURABLE_NOTIFICATION_STATE_INVALID");
                if (entry.value.sessionKey !== binding)
                    continue;
...
const decodeStoredSubscription = (value) => {
    const code = "DURABLE_SUBSCRIPTION_STATE_INVALID";
    if (!isRecord(value) || !exactKeys(value, ["lastCursor", "lastSourceEpoch", "session", "subscriptionId"], ["filter"]))
        throw error(code);
    let filter;
    try {
        filter = validateNotificationFilter(value.filter);
    }
    catch {
        throw error(code);
    }
...
const decodeEvent = (stored) => freezeRecord({
    ...decodeNotification({
...
export class DurableSubscriptionRepository {
    ...
    async subscribe(input) {
        ...
        return runDurableBridgeTransaction(this.#store, "subscription.subscribe", async (transaction) => {
            ...
            if (existing !== null) {
                ...
                if (!equalIdentity(existingSession, input.session))
                    throw error("SUBSCRIPTION_BINDING_MISMATCH");
                if (!sameJson(decoded.filter, filter))
                    throw error("SUBSCRIPTION_STATE_CONFLICT");
    ...
    async unsubscribe(subscriptionId, session) {
    ...
    async publish(subscriptionId, session, record) {
        ...
        return runDurableBridgeTransaction(this.#store, "subscription.publish", async (transaction) => {
            ...
            if (existing !== null && !sameJson(decodeStoredEvent(existing), storedEvent))
                throw error("EVENT_STATE_CONFLICT");
    ...
    async acknowledge(input) {
        ...
        return runDurableBridgeTransaction(this.#store, "subscription.acknowledge", async (transaction) => {
            ...
            if (event.subscriptionId !== input.subscriptionId || !equalIdentity(event.binding, input.session))
                throw error("EVENT_NOT_FOUND");
            if (event.sourceEpoch !== input.sourceEpoch || event.cursor !== input.cursor)
                throw error("EVENT_ACK_INVALID");
            if (!stored.acknowledged)
                await transaction.write("subscription.events", eventKey, { ...stored, acknowledged: true });
            return event;
        });
    }
    async pending(subscriptionId, session) {
        assertSession(session);
        return runDurableBridgeTransaction(this.#store, "subscription.pending", async (transaction) => {
            ...
            for (const entry of await transaction.scan("subscription.events")) {
                const stored = decodeStoredEvent(entry.value);
                if (!stored.acknowledged && stored.event.subscriptionId === subscriptionId)
                    output.push(decodeEvent(stored));
            }
            output.sort(comparePosition);
    ...
    async #validateReopenedState() {
        await runDurableBridgeTransaction(this.store, "bridge.state.validate", async (transaction) => {
            ...
            for (const entry of await transaction.scan("pairing.tickets")) {
                ...
                if (ticket.ticketId !== entry.key)
                    throw error("DURABLE_PAIRING_STATE_INVALID");
                if (entry.value.consumed)
            ...
            for (const entry of await transaction.scan("pairing.bindings")) {
                ...
                if (identityKey(binding) !== entry.key)
                    throw error("DURABLE_PAIRING_STATE_INVALID");
            ...
            for (const entry of await transaction.scan("notification.records")) {
                ...
                if (entry.key !== `${entry.value.sessionKey}\u0000${record.recordId}`)
                    throw error("DURABLE_NOTIFICATION_STATE_INVALID");
            ...
            for (const entry of await transaction.scan("notification.positions")) {
                if (!isRecord(entry.value) || !exactKeys(entry.value, ["cursor", "recordId", "sessionKey", "sourceEpoch"])
                    || entry.value.sessionKey !== entry.key || typeof entry.value.recordId !== "string")
                    throw error("DURABLE_NOTIFICATION_STATE_INVALID");
            ...
            for (const entry of await transaction.scan("subscription.bindings")) {
                ...
                if (binding.subscriptionId !== entry.key)
            ...
            for (const entry of await transaction.scan("subscription.events")) {
                ...
                if (!binding || entry.key !== `${stored.event.subscriptionId}\u0000${stored.event.eventId}`
                    || !equalIdentity(decodeSession(binding.session, "DURABLE_SUBSCRIPTION_STATE_INVALID"), decodeSession(stored.event.binding, "DURABLE_SUBSCRIPTION_STATE_INVALID")))
                    throw error("DURABLE_SUBSCRIPTION_STATE_INVALID");
            }
        });
    }
