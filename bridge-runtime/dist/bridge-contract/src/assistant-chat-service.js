import { BridgeServiceError, freezeRecord, equalIdentity, } from "./service-types.js";
import { OperationDispatcher } from "./operation-dispatch.js";
import { InMemoryAssistantReplyEventStore, } from "./assistant-reply-events.js";
export const ZERO_RETENTION_UNAVAILABLE = "ZERO_RETENTION_UNAVAILABLE";
const MAX_FILES = 4;
...
export const isCurrentZeroRetention = (evidence, now = new Date()) => {
    if (!evidence || !evidence.provider || !evidence.profileId || !evidence.revision)
        return false;
    if (evidence.providerObjectRetention !== "none")
        return false;
    if (!evidence.requestResponseLoggingDisabled || !evidence.trainingDisabled || !evidence.humanReviewDisabled)
        return false;
...
export class AssistantChatService {
            ...
            try {
                if (!this.#respondStream) {
                    ...
                    for await (const delta of response) {
                        if (typeof delta !== "string" || reply.length + delta.length > 50_000)
                            throw new BridgeServiceError("ASSISTANT_REPLY_INVALID");
                        reply += delta;
                        await emit("delta", delta);
                    }
                }
                if (reply.length > 50_000)
                    throw new BridgeServiceError("ASSISTANT_REPLY_INVALID");
                ...
                if (terminal !== "failed") {
                    ...
                    catch { /* The failed event was appended before its sink attempt. */ }
                }
    ...
    async #prepare(request) {
        ...
        if (!isCurrentZeroRetention(request.zeroRetention, new Date(this.#clock())))
            throw new BridgeServiceError(ZERO_RETENTION_UNAVAILABLE);
        ...
        this.#assertSession(request.session);
        if (decision.policyRevision !== request.session.policyAttestationRevision)
            throw new BridgeServiceError("AUTHORIZATION_REVISION_STALE");
        if (!decision.allowed)
            throw new BridgeServiceError(decision.reason ?? "NOT_AUTHORIZED");
        return attachments;
    }
    async #resolveArtifactsForClaim(attachments, session) {
        this.#assertSession(session);
        await this.#assertArtifacts(attachments, session);
        this.#assertSession(session);
    }
    async #assertArtifacts(attachments, session) {
        if (attachments.length === 0)
            return;
        if (!this.#resolveArtifact)
            throw new BridgeServiceError("ARTIFACT_NOT_COMMITTED");
        if (this.#boundConnectionGeneration === null)
        ...
        for (const attachment of attachments) {
            ...
            if (!matching)
                throw new BridgeServiceError("ARTIFACT_FENCE_MISMATCH");
        }
    }
    #complete(request, attachments, reply) {
        const result = freezeRecord({ operationId: request.operationId, messageId: request.messageId, status: "accepted", reply });
    ...
    #validateRequest(request) {
        if (!request.operationId || !request.messageId || typeof request.text !== "string" || request.text.length === 0 || request.text.length > 50_000)
            throw new BridgeServiceError("ASSISTANT_REQUEST_INVALID");
        if (!Array.isArray(request.attachments) && request.attachments !== undefined)
            throw new BridgeServiceError("ATTACHMENT_INVALID");
        const attachments = request.attachments ?? [];
        if (attachments.length > MAX_FILES)
            throw new BridgeServiceError("ATTACHMENT_LIMIT");
        let total = 0;
        const normalized = [];
        for (const attachment of attachments) {
            ...
            if (input.kind !== "image" && input.kind !== "file" && input.kind !== "audio")
                throw new BridgeServiceError("ATTACHMENT_INVALID");
            ...
            if (!Number.isSafeInteger(input.sizeBytes) || input.sizeBytes < 0 || input.sizeBytes > maxBytes)
                throw new BridgeServiceError("ATTACHMENT_INVALID");
            if (input.kind === "image" && !IMAGE_MIME_TYPES.has(input.mimeType))
                throw new BridgeServiceError("ATTACHMENT_UNSUPPORTED");
            if (input.kind === "file" && !FILE_MIME_TYPES.has(input.mimeType))
                throw new BridgeServiceError("ATTACHMENT_UNSUPPORTED");
            if (input.kind === "audio") {
