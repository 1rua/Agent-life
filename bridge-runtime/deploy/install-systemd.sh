...
if [[ -z ${AGENT_LIFE_TSNET_HOSTNAME:-} || -z ${AGENT_LIFE_TSNET_CONTROL_URL:-} ]]; then
  echo 'SYSTEMD_INSTALL_BLOCKED AGENT_LIFE_TSNET_HOSTNAME and AGENT_LIFE_TSNET_CONTROL_URL are required' >&2
  exit 1
fi
if [[ ! -r ${AGENT_LIFE_PAIRING_PUBLIC_KEY:-} || ! -r ${AGENT_LIFE_TSNET_AUTHKEY_FILE:-} ]]; then
  echo 'SYSTEMD_INSTALL_BLOCKED pairing public key and tsnet auth key credential are required' >&2
  exit 1
fi
...
printf 'AGENT_LIFE_TSNET_HOSTNAME=%q\nAGENT_LIFE_TSNET_CONTROL_URL=%q\n' "$AGENT_LIFE_TSNET_HOSTNAME" "$AGENT_LIFE_TSNET_CONTROL_URL" > "$INSTALL_ROOT/etc/agent-life/ingress/ingress.env"
