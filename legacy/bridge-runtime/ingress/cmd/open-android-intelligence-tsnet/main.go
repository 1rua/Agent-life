// Command open-android-intelligence-tsnet is the private Tailnet ingress sidecar. It never
...
const (
	fingerprintHeader = "X-Open-Android-Intelligence-Peer-Fingerprint"
...
func authenticatedProxy(runtimeSocket string) *httputil.ReverseProxy {
	...
	return &httputil.ReverseProxy{
		...
		Rewrite: func(request *httputil.ProxyRequest) {
			...
			outbound.URL.Host = "open-android-intelligence-bridge-runtime"
...
func readConfig() (config, error) {
	socket := os.Getenv("OPEN_ANDROID_INTELLIGENCE_RUNTIME_SOCKET")
	stateDir := os.Getenv("OPEN_ANDROID_INTELLIGENCE_TSNET_STATE_DIR")
	hostname := os.Getenv("OPEN_ANDROID_INTELLIGENCE_TSNET_HOSTNAME")
	control := os.Getenv("OPEN_ANDROID_INTELLIGENCE_TSNET_CONTROL_URL")
	portText := os.Getenv("OPEN_ANDROID_INTELLIGENCE_TSNET_PORT")
	if socket == "" || stateDir == "" {
	...
	if hostname == "" {
		hostname = "open-android-intelligence-bridge"
	}
	if control == "" {
	...
	authKeyPath := os.Getenv("OPEN_ANDROID_INTELLIGENCE_TSNET_AUTHKEY_FILE")
	var authKey string
	if authKeyPath != "" {
		var authErr error
