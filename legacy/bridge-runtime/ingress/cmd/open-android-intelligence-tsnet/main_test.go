...
func TestProxyRewriteRemovesForwardingHeaders(t *testing.T) {
	proxy := authenticatedProxy("/run/open-android-intelligence/runtime.sock")
	...
	if outbound.URL.Scheme != "http" || outbound.URL.Host != "open-android-intelligence-bridge-runtime" {
		t.Fatalf("outbound URL = %s, want http://open-android-intelligence-bridge-runtime", outbound.URL.String())
	}
}

func TestReadConfigDefaults(t *testing.T) {
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_RUNTIME_SOCKET", "/run/open-android-intelligence/runtime.sock")
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_STATE_DIR", "/var/lib/open-android-intelligence-ingress")
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_HOSTNAME", "")
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_CONTROL_URL", "")
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_PORT", "")
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_AUTHKEY_FILE", "")
	cfg, err := readConfig()
	if err != nil {
		t.Fatal(err)
	}
	if cfg.hostname != "open-android-intelligence-bridge" {
...
func TestReadConfigRequiresHTTPSAndSafeCredential(t *testing.T) {
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_RUNTIME_SOCKET", "/run/open-android-intelligence/runtime.sock")
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_STATE_DIR", "/var/lib/open-android-intelligence-ingress")
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_HOSTNAME", "open-android-intelligence-bridge")
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_PORT", "443")
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_CONTROL_URL", "http://control.invalid")
	path := filepath.Join(t.TempDir(), "auth-key")
	if err := os.WriteFile(path, []byte("key"), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_AUTHKEY_FILE", path)
	if _, err := readConfig(); err == nil || !strings.Contains(err.Error(), "HTTPS") {
		t.Fatalf("readConfig() error = %v, want HTTPS rejection", err)
	}
	t.Setenv("OPEN_ANDROID_INTELLIGENCE_TSNET_CONTROL_URL", "https://control.invalid")
